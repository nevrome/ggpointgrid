...
arrange_points_on_grid <- function(tab, grid_x, grid_y) {
  ...
  grid_df <- core_arrange_algorithm2(distance_long)
...
core_arrange_algorithm3 <- function(distance_long, distance_long_is_sorted = FALSE) {
  ...
  if (nrow(new_distance_long) == 0) {
    ...
    dplyr::bind_rows(
      core_arrange_algorithm3(new_distance_long, distance_long_is_sorted = T),
      newly_solved
    )
  }
}
